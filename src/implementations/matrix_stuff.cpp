...
// NEEDS TESTING
// New implementation using passed jacobian matrix and new Matrix_Block structure
void matrix_blocks_times_vector(Matrix_Block *block_list, int num_blocks, double *jacobian, double *vector, double *result)
{
    ...
    for (int i = 0; i < num_blocks; i++)
    {
        ...
        Matrix_Block& current_block = block_list[i];
        const int rows = current_block.rows;
        const int cols = current_block.cols;
        const int row_origin = current_block.row;
        const int col_origin = current_block.col;
        double *matrix = &jacobian[current_block.jacobian_index];
        ...
        for (int j = 0; j < block_list[i].rows * block_list[i].cols; j++)
        {
            ...
            // Relative row number.
            // Find column im in IN THIS BLOCK.
            int relative_row = j / cols;
            int relative_col = j % cols;

            // Instead of editing the current element in a block matrix, we edit the current position in the vector
            result[row_origin + relative_row] += matrix[j] * vector[col_origin + relative_col];
        }
    }
}

// WORKING WITH OLD CLASS
// Multiplies a list of matrix blocks times a vector. Results in a vector
//void matrix_blocks_times_vector_old(Matrix_Block *block_list, int num_blocks, double *vector, double *result)
//{
//	// Loop through input blocks
//	for (int i = 0; i < num_blocks; i++)
//	{
//		// Each matrix block represents a certain number of rows and columns in the matrix
//		int block_rows = block_list[i].rows;
//		int block_cols = block_list[i].cols;
//		double *block_matrix = block_list[i].matrix;
//		int block_row_start = block_list[i].row;
//		int block_col_start = block_list[i].col;
//
//		// Each matrix block represents a certain number of rows and columns in the matrix
//		for (int j = 0; j < block_rows * block_cols; j++)
//		{
//			// Relative row number
//			int relRow = j / block_cols;
//			// Find column im in IN THIS BLOCK.
//			int relCol = j % block_cols;
//
//			// The indice of the resulting vector that gets edited is the same as the row of the element in the matrix
//			result[block_row_start + relRow] += block_matrix[j] * vector[block_col_start + relCol];
//		}
//	}
//}


// New Matrix Block Style: Jacobian 1d array pointer is passed
void matrix_blocks_transpose_times_vector(Matrix_Block *block_list, int num_blocks, double *jacobian, double *vector, double *result)
{
    ...
    for (int i = 0; i < num_blocks; i++)
    {
        ...
        for (int j = 0; j < block_list[i].rows * block_list[i].cols; j++)
        {
            ...
            // Instead of editing the current element in a block matrix, we edit the current position in the vector
            result[col_origin + relative_col] += matrix[j] * vector[row_origin + relative_row];
        }
    }
}
...
//void matrix_blocks_transpose_times_vector_old(Matrix_Block *block_list, int num_blocks, double *vector, double *result)
//{
//	// Loop through input blocks
//	for (int i = 0; i < num_blocks; i++)
//	{
...
//
//			// Instead of editing the current element in a block matrix, we edit the current position in the vector
//			result[col_origin + relative_col] += current_block.matrix[j] * vector[row_origin + relative_row];
//		}
//	}
//}
...
// Post optimization version using std::max with a custom lambda comparator
double calculate_relevant_norm(double *v, int n, int itol)
{
	...
	{
		// THIS IS COOL! It sets current max element to a, next element to b, if |a| < |b| it returns true for changing the maximum. Well optimized!
		double *max_element_it = std::max_element(v, v + n, [](double a, double b) {
			return std::fabs(a) < std::fabs(b);
		});
		double max_element = *max_element_it;
		return std::fabs(max_element);
	}
}
// Pre change version:
...
// New implementation for new Matrix Block Class: NEEDS TESTING
...
void A_times_x(State_Getter *state, double *x, double *result)
{
	...
	double *first_res = new double[state->num_bodies * 2]();
	matrix_blocks_transpose_times_vector(state->jacobian, state->num_constraints, state->jacobian_data, x, first_res);

	// Then multiply mass inverse times first_res
	double *second_res = new double[state->num_bodies * 2]();
	diagonal_times_vector(state->inverse_mass_matrix, first_res, 2 * state->num_bodies, second_res);

	// Then multiply the jacobian times the second result, and thats it
	matrix_blocks_times_vector(state->jacobian, state->num_constraints, state->jacobian_data, second_res, result);

	delete[] first_res;
	delete[] second_res;
}

// Working with OLD MATRIX BLOCK CLASS
//// Function that calculates A*x: A is always symmetric in our problem. A= J* M^-1 *J_T
//// Symmetric means A_T = A
//void A_times_x(State_Getter *state, double *x, double *result)
//{
...
//	zero_vector(result, state->num_constraints);
//	// First multiply the jacobian transpose times x
//	double *first_res = new double[state->num_bodies * 3]();
//	matrix_blocks_transpose_times_vector(state->jacobian, state->num_constraints, x, first_res);
//
//	// Then multiply mass inverse times first_res
//	double *second_res = new double[state->num_bodies * 3]();
...
{
    std::memset(vector, 0, length * sizeof(double));
}

void ones_vector(double *vector, int length)
{
    std::memset(vector, 1, length * sizeof(double));
}
